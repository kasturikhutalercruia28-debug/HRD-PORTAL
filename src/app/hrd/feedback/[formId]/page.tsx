...
const COLORS = ["#AAFF47", "#0D0D0B", "#6b7280", "#d1d5db", "#f3f4f6"];
...
export default function HrdFeedbackFormPage() {
  ...
  if (!form) return <div className="p-8 text-center text-[#0D0D0B]/40 text-sm">Loading…</div>;

  const TABS = ["overview", "questions", "responses", "analytics"] as const;

  return (
    <div className="max-w-4xl mx-auto p-6">
      <Link href="/hrd/feedback" className="inline-flex items-center gap-1.5 text-sm text-[#0D0D0B]/50 hover:text-[#0D0D0B] mb-4 transition-colors">
        <ArrowLeft size={14} /> Back
      </Link>

      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          <h1 className="font-['Fraunces'] text-2xl font-bold text-[#0D0D0B]">{form.eventName}</h1>
          <p className="text-sm text-[#0D0D0B]/50 mt-0.5">
        ...
        <div className="flex items-center gap-2">
          <a href={`/api/export/feedback/${formId}?format=csv`} className="text-xs border border-black/15 text-[#0D0D0B] px-3 py-1.5 rounded-lg hover:bg-black/5 transition-colors flex items-center gap-1.5">
            <Download size={12} /> CSV
          </a>
          <a href={`/api/export/feedback/${formId}?format=xlsx`} className="text-xs bg-[#AAFF47] text-[#0D0D0B] px-3 py-1.5 rounded-lg hover:bg-[#99ee36] transition-colors font-semibold flex items-center gap-1.5">
      ...
      <div className="flex gap-1 border-b border-black/10 mb-6">
        {TABS.map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm font-medium capitalize transition-colors ${tab === t ? "border-b-2 border-[#AAFF47] text-[#0D0D0B]" : "text-[#0D0D0B]/40 hover:text-[#0D0D0B]"}`}
          >
            {t}
          </button>
        ))}
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={label} className="bg-white rounded-xl border border-black/5 p-4">
                <p className="text-xs text-[#0D0D0B]/50">{label}</p>
                <p className="text-xl font-bold text-[#0D0D0B] mt-1">{value}</p>
              </div>
            ))}
          </div>
          {form.feedbackOpenAt && (
            <p className="text-sm text-[#0D0D0B]/60">Opens: {new Date(form.feedbackOpenAt).toLocaleString("en-IN")}</p>
          )}
          {form.feedbackCloseAt && (
            <p className="text-sm text-[#0D0D0B]/60">Closes: {new Date(form.feedbackCloseAt).toLocaleString("en-IN")}</p>
          )}
        </div>
      )}
      ...
      {tab === "questions" && (
        <div className="space-y-4">
          {form.questions.map((q, i) => (
            <div key={q.id} className="bg-white rounded-xl border border-black/5 p-4 flex items-start justify-between gap-3">
              <div className="flex-1">
                <p className="text-sm font-medium text-[#0D0D0B]">{i + 1}. {q.questionText}</p>
                <p className="text-xs text-[#0D0D0B]/40 mt-0.5 capitalize">{q.questionType.replace("_", " ")} {q.isRequired ? "· required" : "· optional"}</p>
                {q.options && q.options.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1.5">
                    {q.options.map((o) => <span key={o} className="text-xs bg-[#F0EDE5] text-[#0D0D0B]/70 px-2 py-0.5 rounded">{o}</span>)}
                  </div>
                )}
              </div>
          ...
          <form onSubmit={addQuestion} className="bg-white rounded-xl border border-dashed border-black/20 p-4 space-y-3">
            <p className="text-xs font-semibold text-[#0D0D0B]">Add Question</p>
            <input
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 text-sm text-[#0D0D0B] cursor-pointer">
                <input type="checkbox" checked={newQ.isRequired} onChange={(e) => setNewQ((q) => ({ ...q, isRequired: e.target.checked }))} className="accent-[#AAFF47]" />
                Required
              </label>
              <button type="submit" disabled={saving} className="bg-[#AAFF47] text-[#0D0D0B] px-4 py-1.5 rounded-lg text-sm font-semibold hover:bg-[#99ee36] transition-colors disabled:opacity-50 flex items-center gap-1.5">
                <Plus size={14} /> Add
              </button>
            </div>
          </form>
      ...
      {tab === "responses" && (
        <div className="space-y-4">
          {submissions.length === 0 ? (
            <div className="bg-white rounded-xl border border-black/5 p-12 text-center text-[#0D0D0B]/40 text-sm">No submissions yet</div>
          ) : (
            submissions.map((sub) => (
              <div key={sub.id} className="bg-white rounded-xl border border-black/5 p-4">
                <p className="text-xs font-semibold text-[#0D0D0B]">{sub.submitter.name} · {sub.submitter.role}</p>
                <p className="text-[10px] text-[#0D0D0B]/40 mb-3">{new Date(sub.submittedAt).toLocaleString("en-IN")}</p>
                <div className="space-y-1.5">
                  {sub.responses.map((r) => (
                    <div key={r.questionId} className="flex gap-2">
                      <span className="text-xs text-[#0D0D0B]/50 flex-1 min-w-0">{r.question.questionText}:</span>
                      <span className="text-xs font-medium text-[#0D0D0B]">{r.answer}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="space-y-6">
          {form.questions.filter((q) => ["star_rating", "yes_no", "multiple_choice"].includes(q.questionType)).map((q) => (
            <div key={q.id} className="bg-white rounded-xl border border-black/5 p-5">
              <p className="text-sm font-semibold text-[#0D0D0B] mb-4">{q.questionText}</p>
              {q.questionType === "star_rating" && (
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={getStarData(q.id)}>
                    ...
                    <Tooltip />
                    <Bar dataKey="count" fill="#AAFF47" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              )}
                  ...
                  <div className="space-y-1.5">
                    {getPieData(q.id).map(({ name, value }, i) => (
                      <div key={name} className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-[#0D0D0B]">{name}: <strong>{value}</strong></span>
                      </div>
                    ))}
                  </div>
                </div>
