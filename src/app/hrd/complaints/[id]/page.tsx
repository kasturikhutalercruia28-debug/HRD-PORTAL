...
export default function HrdComplaintDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/complaints/${id}`)
      .then((r) => r.json())
      .then((data) => { setComplaint(data); setStatus(data.status); });
  }, [id]);
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    if (res.ok) {
      router.refresh();
      fetch(`/api/complaints/${id}`).then((r) => r.json()).then((data) => { setComplaint(data); setRemark(""); });
    } else {
  ...
  if (!complaint) return <div className="p-8 text-center text-[#0D0D0B]/40 text-sm">Loading…</div>;

  return (
    <div className="max-w-2xl mx-auto p-6">
      <Link href="/hrd/complaints" className="inline-flex items-center gap-1.5 text-sm text-[#0D0D0B]/50 hover:text-[#0D0D0B] mb-4 transition-colors">
        <ArrowLeft size={14} /> Back
      </Link>

      <div className="bg-white rounded-xl border border-black/5 p-6 mb-4">
        <div className="flex items-start justify-between gap-4 mb-3">
          <h1 className="font-['Fraunces'] text-xl font-bold text-[#0D0D0B]">{complaint.subject}</h1>
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium shrink-0 ${STATUS_COLORS[complaint.status]}`}>
            {complaint.status.replace("_", " ")}
          </span>
        </div>
        <p className="text-xs text-[#0D0D0B]/50 mb-3">
          {complaint.submitter.name} · {complaint.submitter.role} ·{" "}
          {new Date(complaint.createdAt).toLocaleString("en-IN")}
        </p>
        <p className="text-sm text-[#0D0D0B]/70 whitespace-pre-wrap">{complaint.description}</p>
      </div>

      {/* Update form */}
      <div className="bg-white rounded-xl border border-black/5 p-5 mb-4">
        <h2 className="text-sm font-semibold text-[#0D0D0B] mb-3">Update Status</h2>
      ...
      {complaint.history.length > 0 && (
        <div>
          <h2 className="text-sm font-semibold text-[#0D0D0B] mb-2">History</h2>
            ...
            {complaint.history.map((h) => (
              <div key={h.id} className="bg-white rounded-lg border border-black/5 p-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-[#0D0D0B]/50">by {h.updatedBy.name}</span>
                </div>
                {h.remark && <p className="text-sm text-[#0D0D0B]/70 mt-1">{h.remark}</p>}
                <p className="text-[10px] text-[#0D0D0B]/40 mt-1">{new Date(h.createdAt).toLocaleString("en-IN")}</p>
              </div>
            ))}
          </div>
        </div>
