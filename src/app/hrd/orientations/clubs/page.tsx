  ...
  return (
    <div className="p-6 lg:p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-['Fraunces'] text-2xl font-bold text-[#0D0D0B]">Clubs</h1>
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold font-['Geist'] transition-colors border ${
              mode === "bulk"
                ? "bg-[#0D0D0B] text-[#AAFF47] border-[#0D0D0B]"
                : "bg-white border-black/10 text-[#0D0D0B] hover:border-black/20"
            }`}
          >
            <List size={14} />
            Bulk Add
      ...
      {mode === "single" && (
        ...
        >
          <h2 className="font-['Fraunces'] font-semibold text-[#0D0D0B]">Add New Club</h2>
          {error && <p className="text-red-600 text-sm font-['Geist']">{error}</p>}
          <div className="grid gap-3 sm:grid-cols-3">
            <div>
              <label className="block text-xs text-[#0D0D0B]/50 mb-1 font-['Geist']">Club Name</label>
              <input
                required
                value={form.name}
            ...
            <div>
              <label className="block text-xs text-[#0D0D0B]/50 mb-1 font-['Geist']">Login Email</label>
              <input
                required
                type="email"
            ...
            <div>
              <label className="block text-xs text-[#0D0D0B]/50 mb-1 font-['Geist']">Password</label>
              <input
                required
                type="password"
      ...
      {mode === "bulk" && (
        <form
          ...
          className="bg-white rounded-xl border border-black/5 shadow-sm p-5 space-y-4"
        >
          <h2 className="font-['Fraunces'] font-semibold text-[#0D0D0B]">Bulk Add Clubs</h2>
          <p className="text-[#0D0D0B]/50 text-xs font-['Geist']">
            Paste one club name per line. Login emails are auto-generated; you can change them after.
          </p>

          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              <label className="block text-xs text-[#0D0D0B]/50 mb-1 font-['Geist']">
              ...
              <textarea
                ...
                className="w-full border border-black/10 rounded-lg px-3 py-2 text-sm font-['Geist'] focus:outline-none focus:ring-2 focus:ring-[#AAFF47] resize-none"
            ...
            <div className="space-y-3">
              ...
              {bulkPreview.length > 0 && (
                <div>
                  <p className="text-xs text-[#0D0D0B]/50 font-['Geist'] mb-2">
                  ...
                  <div className="max-h-48 overflow-y-auto border border-black/10 rounded-lg divide-y divide-black/5">
                    {bulkPreview.map(({ name, email }) => (
                      <div key={name} className="px-3 py-2">
                        <p className="text-xs font-medium text-[#0D0D0B] font-['Geist']">{name}</p>
                        <p className="text-[10px] text-[#0D0D0B]/40 font-['Geist']">{email}</p>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {bulkResult && (
            <div className={`rounded-lg px-4 py-3 text-sm font-['Geist'] ${bulkResult.failed.length === 0 ? "bg-[#AAFF47]/10 text-[#0D0D0B]" : "bg-amber-50 text-amber-800"}`}>
              ...
              {clubs.map((club) => (
                <div key={club.id} className="flex items-center gap-4 px-6 py-4">
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-[#0D0D0B] font-['Geist']">{club.name}</p>
                  ...
                  >
                    {club.isActive ? (
                      <ToggleRight size={22} className="text-[#AAFF47]" />
                    ) : (
                      <ToggleLeft size={22} />
                    )}
                  </button>
                  <span className={`text-xs font-['Geist'] w-14 text-right ${club.isActive ? "text-emerald-600" : "text-[#0D0D0B]/30"}`}>
