...
export default function OcvsPage() {
  const [clubs, setClubs] = useState<Club[]>([]);
  const [dcms, setDcms] = useState<Dcm[]>([]);
  const [records, setRecords] = useState<OcvRecord[]>([]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    const res = await fetch("/api/hrd/criteria/ocvs", {
      method: "POST",
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto space-y-6">
      ...
      <div>
        <h1 className="font-['Fraunces'] text-2xl font-bold text-[#180F04]">OCVs</h1>
        <p className="text-[#180F04]/60 text-sm mt-1">Mark which DCMs attended a club's OCV.</p>
      </div>
      ...
      <div className="bg-white rounded-xl border border-black/5 overflow-hidden">
        ...
        {records.length === 0 ? (
          <p className="px-5 py-6 text-xs text-[#180F04]/40 text-center">No OCVs recorded yet.</p>
        ) : (
