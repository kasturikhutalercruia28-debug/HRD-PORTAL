...
export default function DECLayout({ children }: { children: React.ReactNode }) {
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {NAV_LINKS.map(({ href, label, icon: Icon }) => (
          ...
          >
            ...
            {label}
            {isActive(href) && (
              <ChevronRight size={14} className="ml-auto text-[#AAFF47]/60" />
            )}
          </Link>
        ))}
      </nav>
  ...
  return (
    <div className="flex h-screen bg-[#F0EDE5] font-['Geist']">
      {/* Desktop sidebar */}
      <aside className="hidden lg:flex flex-col w-64 bg-[#0D0D0B] flex-shrink-0">
        <SidebarContent />
      </aside>
      ...
      <aside
        className={`
          fixed top-0 left-0 h-full w-64 bg-[#0D0D0B] z-50 transform transition-transform duration-200
          lg:hidden
