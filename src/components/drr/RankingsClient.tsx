...
function CategoryBadge({ category }: { category: string }) {
  if (category === "elite") {
    return (
      <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-[#AAFF47]/20 text-[#0D0D0B] border border-[#AAFF47]/40 uppercase tracking-wide font-['Geist']">
        Elite
      </span>
    );
  }
    ...
    <div className="p-6 lg:p-8 space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        {availableQuarters.length > 0 && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-[#0D0D0B]/50 font-['Geist']">Quarter:</span>
      ...
      <div className="flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-md text-xs font-semibold font-['Geist'] transition-colors ${
                activeTab === key
                  ...
                  : "text-[#0D0D0B]/50 hover:text-[#0D0D0B]"
              }`}
            >
              {label}
              <span
                className={`ml-1.5 text-[10px] ${activeTab === key ? "text-[#0D0D0B]/50" : "text-[#0D0D0B]/30"}`}
              >
                {count}
              </span>
            </button>
        ...
        <select
          ...
          className="appearance-none bg-white border border-black/10 rounded-lg px-3 py-1.5 text-xs font-['Geist'] text-[#0D0D0B] focus:outline-none focus:ring-2 focus:ring-[#AAFF47]/50"
        >
        ...
        <span className="ml-auto text-xs text-[#0D0D0B]/40 font-['Geist']">
          ...
          <table className="w-full text-sm font-['Geist'] min-w-[900px]">
            <thead>
              <tr className="border-b border-black/5 bg-[#F0EDE5]/50">
                <th className="text-center px-4 py-3 text-[#0D0D0B]/50 font-medium text-xs uppercase tracking-wide w-12">
                  Rank
                </th>
                <th className="text-left px-4 py-3 text-[#0D0D0B]/50 font-medium text-xs uppercase tracking-wide">
                  Name
                </th>
                <th className="text-left px-3 py-3 text-[#0D0D0B]/50 font-medium text-xs uppercase tracking-wide">
                  Avenue
                </th>
                {quarterMonths.map((m) => (
                  <th key={m} className="text-center px-3 py-3 text-[#0D0D0B]/50 font-medium text-xs uppercase tracking-wide whitespace-nowrap">
                    {MONTH_NAMES[m]}
                  </th>
                ))}
                <th className="text-center px-3 py-3 text-[#0D0D0B]/50 font-medium text-xs uppercase tracking-wide whitespace-nowrap">
                  Q Avg
                </th>
              ...
              {displayed.map((audit, idx) => (
                ...
                >
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold font-['Fraunces'] ${
                        audit.rank === 1
                          ? "bg-[#AAFF47] text-[#0D0D0B]"
                          : audit.rank === 2
                          ? "bg-gray-200 text-[#0D0D0B]"
                          : audit.rank === 3
                          ? "bg-amber-200 text-amber-900"
                          : "bg-black/5 text-[#0D0D0B]/60"
                      }`}
                    >
                      {audit.rank}
                    </span>
                  ...
                  <td className="px-4 py-3">
                    <p className="font-semibold text-[#0D0D0B]">{audit.name}</p>
                    <p className="text-[#0D0D0B]/40 text-xs">{audit.title}</p>
                  </td>
                  ...
                  <td className="px-3 py-3 text-center">
                    <span
                      className={`text-xs font-semibold ${
                        audit.adjustment > 0
                          ? "text-[#AAFF47] bg-[#AAFF47]/10 px-1.5 py-0.5 rounded"
                          : audit.adjustment < 0
                  ...
                  <td className="px-3 py-3 text-center font-semibold text-[#0D0D0B]">
